...
static inline GICR_IPRIORITYR_VALUE_STRUCT_NAME GICV3_GICR_IPRIORITYR_read(
	uintptr base, size_t rd, size_t n)
{
...
static inline void GICV3_GICR_IPRIORITYR_write(
	uintptr base, size_t rd, size_t n, GICR_IPRIORITYR_VALUE_STRUCT_NAME v)
{
	*((reg32_ptr)(GICV3_SGI_BASE(base, rd) + GICR_IPRIORITYR_OFFSET(n))) =
		v.val;
}

static inline uint8 GICV3_GICR_IPRIORITYR_BF_get(
	const GICR_IPRIORITYR_VALUE_STRUCT_NAME r, size_t byte_idx)
{
	if (byte_idx > 3) PANIC("GICR_IPRIORITYR: byte_idx index must be <= 3");
...
static inline void GICV3_GICR_IPRIORITYR_BF_set(
	GICR_IPRIORITYR_VALUE_STRUCT_NAME *r, size_t byte_idx, uint8 priority)
{
	if (byte_idx > 3) PANIC("GICR_IPRIORITYR: byte_idx index must be <= 3");
