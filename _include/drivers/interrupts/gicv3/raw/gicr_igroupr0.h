...
static inline GICR_IGROUPR0_VALUE_STRUCT_NAME GICV3_GICR_IGROUPR0_read(uintptr base, size_t n)
{
	return (GICR_IGROUPR0_VALUE_STRUCT_NAME) {
		       .val = *((reg32_ptr)(GICV3_SGI_BASE(base, n) + (GICR_IGROUPR0_OFFSET)))
	};
}
...
/* Helpers */
static inline void GICV3_GICR_IGROUPR0_set_bit(GICR_IGROUPR0_VALUE_STRUCT_NAME *r, size_t n, bool v)
{
#ifdef TEST
	if (n >= 32)
		PANIC("Invalid GICR_IGROUPR0 bit");
#endif
...
static inline bool GICV3_GICR_IGROUPR0_get_bit(GICR_IGROUPR0_VALUE_STRUCT_NAME r, size_t n)
{
#ifdef TEST
	if (n >= 32)
		PANIC("Invalid GICR_IGROUPR0 bit");
